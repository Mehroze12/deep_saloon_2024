...
const RadioButton = ({
    ...
    return (
        <div>
            {
                auth ?
                    <div onClick={onSelect} className={`w-full cursor-pointer mt-2 p-4 flex items-center justify-between border-[1px]  ${value ? " border-goldColor" : "border-borderColor"} `}>
                        <div className='  flex-col items-center'>
                            <p className={`text8 ${value ? "text-goldColor " : "text-white"}`}> {label} </p>
                            {desc && <p className='text11 text-white w-[70%]  pt-2'> {desc} </p>}
                        </div>
